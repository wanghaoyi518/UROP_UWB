...
void Display_SwitchInfo(u8 value)
{
	...
	printf("Mini4 配置信息：");
	n = sprintf((char*)&SendBuff[0], "Mini4 配置信息：");	
...
{ 
  ...
  //delay_init();	    	 //延时函数初始化
	...
	SPI_Configuration();
	peripherals_init();
	TIM2_Int_Init(999,729);//
	Sleep(1000);
	Flash_Configuration();
	Sleep(200);
	USB_Config();
	
 	MYDMA_Config(DMA1_Channel4,(u32)&USART1->DR,(u32)DMA_USART_Buff,3000);//DMA1通道4,外设为串口1,存储器为SendBuff,长度130.  
 	
	printf("|*******************(C) COPYRIGHT 2021 研创物联*******************\r\n");
	printf("|                                                                 \r\n");
	printf("|            UWB Mini4    -   RTLS (Version 20210601)             \r\n");
	printf("|            YanChuang IOT ltd                                    \r\n");	
	printf("|                                                                 \r\n");	
	printf("|*******************(C) COPYRIGHT 2021 研创物联*******************\r\n");			
	printf("Mini4 开机次数：%d\r\n"    ,sys_para.start_count);

	n = sprintf((char*)&SendBuff[0], "|*******************(C) COPYRIGHT 2021 研创物联*******************\r\n");
	n+=	sprintf((char*)&SendBuff[n], "|                                                                 \r\n");
	n+=	sprintf((char*)&SendBuff[n], "|            UWB Mini4    -   RTLS (Version 20210601)             \r\n");
	n+=	sprintf((char*)&SendBuff[n], "|            YanChuang IOT ltd                                    \r\n");
	n+=	sprintf((char*)&SendBuff[n], "|                                                                 \r\n");
	n+=	sprintf((char*)&SendBuff[n], "|*******************(C) COPYRIGHT 2021 研创物联*******************\r\n");	
	USB_TxWrite(SendBuff, n);
	
	USART_DMACmd(USART1,USART_DMAReq_Tx,ENABLE); //使能串口1的DMA发送  
	MYDMA_Enable(DMA1_Channel4,n);//开始一次DMA传输！	
...
//														(instance_mode == TAG)?'t':'a', taddr, aaddr);
								if(m+65<3000)
								{
									memcpy((u8*)DMA_USART_Buff+m,(u8*)SendBuff,65);
									m+=65;
								}
...
//							memset(SendBuff,0,130);	
              ...
              n = sprintf((char*)&SendBuff[0], "ma %02x %08x %08x %08x %08x %04x %02x %08x a0:%d\r\n",
							...
							/*n = sprintf((char*)&SendBuff[0], "anchor to anchor: ma %02x %08x %08x %08x %08x %04x %02x %08x a0:%d\r\n",
												valid, instancegetidist_mm(0), instancegetidist_mm(1),
						  								instancegetidist_mm(2), instancegetidist_mm(3),
							  							l, instancegetrnumanc(0), rangeTime, aaddr);*/
							
							
							//output multi-digit code
							/*n = sprintf((char*)&SendBuff[0], "anchor to anchor: ma %02x %08x %08x %08x %08x %04x %02x %08x a0:%d  Extra: %lld %lld %lld %lld \r\n",
											valid, instancegetidist_mm(0), instancegetidist_mm(1),instancegetidist_mm(2), instancegetidist_mm(3), l, instancegetrnumanc(0), rangeTime, aaddr, 
											instance_data[0].txu.txTimeStamp, instance_data[0].txu.tagPollTxTime, instance_data[0].txu.anchorRespTxTime, instance_data[0].tagPollRxTime);*/
							
							/*n = sprintf((char*)&SendBuff[0], "anchor to anchor: ma %02x %08x %08x %08x %08x %04x %02x %08x a0:%d  Extra: %lld %lld %lld %lld \r\n",
											valid, instancegetidist_mm(0), instancegetidist_mm(1),instancegetidist_mm(2), instancegetidist_mm(3), l, instancegetrnumanc(0), rangeTime, aaddr, 
											instance_data[0].txu.tagPollTxTime, instance_data[1].txu.anchorRespTxTime, instance_data[2].txu.anchorRespTxTime, instance_data[0].tagPollRxTime);*/
							//tofAnc[tof_idx]//all are 0
							/*n = sprintf((char*)&SendBuff[0], "anchor to anchor: ma %02x %08x %08x %08x %08x %04x %02x %08x a0:%d  Extra: %lu %lu %lu %lu \r\n",
											valid, instancegetidist_mm(0), instancegetidist_mm(1),instancegetidist_mm(2), instancegetidist_mm(3), l, instancegetrnumanc(0), rangeTime, aaddr, 
											instance_data[0].tofAnc[0], instance_data[0].tofAnc[1], instance_data[0].tofAnc[2], instance_data[0].tofAnc[3]);    */
							//tofArrayAnc Repeatly "Extra: 2882404351 2882404351 2882404351 2882404351" 
							/*n = sprintf((char*)&SendBuff[0], "anchor to anchor: ma %02x %08x %08x %08x %08x %04x %02x %08x a0:%d  Extra: %lu %lu %lu %lu \r\n",
											valid, instancegetidist_mm(0), instancegetidist_mm(1),instancegetidist_mm(2), instancegetidist_mm(3), l, instancegetrnumanc(0), rangeTime, aaddr, 
											instance_data[0].tofArrayAnc[0], instance_data[0].tofArrayAnc[1], instance_data[0].tofArrayAnc[2], instance_data[0].tofArrayAnc[3]);*/
            ...
            instancecleardisttableall();
				...
				if(n > 0)
       	{
       		//if((pauseTWRReports == 0)&&(timer2_udma_send >=2))
					if(pauseTWRReports == 0)
       		{
							
							MYDMA_Enable(DMA1_Channel4,m);//开始一次DMA传输！	
//							n=0;
							timer2_udma_send=0;
							m=0;
       		}
