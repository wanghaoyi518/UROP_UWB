...
int reportTOF(int idx, uint32 tofx)//make it signed, divided by 4, multi lightspeed
{
        double distance ;
...
void instance_config(instanceConfig_t *config, sfConfig_t *sfConfig)
{
    ...
    {
        power = txSpectrumConfig[config->channelNumber].txPwr[instance_data[instance].configData.smartPowerEn][config->pulseRepFreq- DWT_PRF_16M];
    }
...
int instancegetidist_mm(int idx) //get instantaneous range
{
    ...
    idx &= (MAX_ANCHOR_LIST_SIZE - 1);//4-1=3
